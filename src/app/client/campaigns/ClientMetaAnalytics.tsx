...
export default function ClientMetaAnalytics({ 
  ...
  return (
    <div className="bg-white border text-sm border-gray-100 shadow-sm rounded-xl p-10 mb-8 mt-2 relative overflow-hidden" ref={reportRef}>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-12 border-b border-gray-50 pb-8 relative z-10">
        <div className="flex items-center gap-5">
           ...
           <div>
             <h2 className="text-3xl font-bold text-gray-900 tracking-tight font-bold">Meta Analytics Engine</h2>
             <p className="text-gray-400 text-[10px] font-semibold text-sm mt-1">Real-time Performance Telemetry • {datePreset.replace('_', ' ')}</p>
           </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4 w-full md:w-auto" data-html2canvas-ignore="true">
          <select 
            ...
            className="px-6 py-3.5 border border-gray-100 rounded-2xl focus:outline-none focus:ring-8 focus:ring-red-50 bg-gray-50 cursor-pointer font-bold text-[10px] uppercase tracking-normal text-gray-900 w-full sm:w-auto shadow-inner transition-all hover:bg-white"
          >
          ...
          {data && !isSharedView && (
            <div className="flex gap-4">
              <button 
                onClick={handleShare}
                className="px-6 py-3.5 bg-white border border-gray-100 text-gray-500 hover:text-black hover:border-black rounded-2xl font-bold text-[10px] uppercase tracking-normal flex items-center justify-center transition-all shadow-sm active:scale-95 group"
              >
              ...
              <button 
                ...
                className="px-8 py-3.5 bg-black hover:bg-blue-600 text-white rounded-2xl font-bold text-[10px] uppercase tracking-normal flex items-center justify-center transition-all shadow-xl hover:shadow-blue-200 disabled:opacity-50 active:scale-95"
              >
      ...
      {isLoading ? (
        <div className="py-40 flex flex-col items-center justify-center text-gray-300">
          ...
          <p className="font-bold text-[10px] uppercase tracking-wider italic">Decrypting Meta Data Stream...</p>
        </div>
      ) : needsOauth ? (
        <div className="py-32 flex flex-col items-center justify-center text-center bg-gray-50/50 rounded-xl border border-gray-100 mx-auto max-w-2xl px-12 relative overflow-hidden group">
          ...
          <div className="w-24 h-24 bg-black rounded-xl shadow-2xl flex items-center justify-center mb-10 rotate-3 group-hover:rotate-12 transition-transform relative z-10">
            <Target className="w-12 h-12 text-white" />
          </div>
          <h3 className="text-3xl font-bold text-gray-900 mb-4 tracking-tight font-bold relative z-10">Link Meta Repository</h3>
          <p className="text-gray-400 mb-12 max-w-sm font-bold text-[10px] uppercase tracking-normal leading-relaxed relative z-10">
            Authenticate your Meta Business assets to initialize the Creova Analytics Matrix and unlock real-time ROAS telemetry.
          </p>
          <button 
            onClick={handleConnect}
            className="bg-black hover:bg-blue-600 text-white px-12 py-5 rounded-2xl text-[10px] font-semibold text-sm flex items-center shadow-2xl transition-all active:scale-95 relative z-10"
          >
            Secure Authentication <ArrowRight className="ml-3 w-4 h-4" />
          </button>
        </div>
      ) : error ? (
        <div className="py-32 flex flex-col items-center justify-center text-center bg-red-50 rounded-xl border border-red-100 max-w-2xl mx-auto shadow-inner">
          <AlertCircle className="w-16 h-16 text-blue-600 mb-6 animate-pulse" />
          <p className="text-blue-600 font-bold text-xl uppercase italic tracking-tight">Matrix Disconnect: {error}</p>
        </div>
      ) : data ? (
        <div className="space-y-10 animate-in fade-in duration-700">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-8">
            ...
            <MetricCard 
              ...
              icon={<Zap className="text-blue-600 w-4 h-4"/>} 
              change={0}
            />
            <MetricCard 
          ...
          <div className="grid grid-cols-1 xl:grid-cols-3 gap-10">
             ...
             <div className="xl:col-span-2 bg-white border border-gray-100 p-10 rounded-xl shadow-sm relative overflow-hidden group">
                ...
                <h3 className="text-[10px] font-medium text-[11px] uppercase tracking-wider text-gray-300 mb-10 flex items-center italic">
                  <Activity className="w-5 h-5 mr-4 text-blue-600" /> Investment Trajectory
                </h3>
             ...
             <div className="bg-black border border-gray-800 p-10 rounded-xl shadow-2xl relative overflow-hidden group">
                <div className="absolute bottom-0 left-0 w-32 h-32 bg-blue-600 rounded-full blur-3xl opacity-10 -ml-16 -mb-16"></div>
                <h3 className="text-[10px] font-medium text-[11px] uppercase tracking-wider text-gray-400 mb-10 flex items-center italic">
                   <Target className="w-5 h-5 mr-4 text-blue-600" /> Efficiency Nodes
                </h3>
                <div className="h-80 w-full relative z-10">
                   <ResponsiveContainer width="100%" height="100%">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
             {/* Age & Gender Coverage */}
             <div className="bg-white border border-gray-100 p-10 rounded-xl shadow-sm">
                <h3 className="text-[10px] font-medium text-[11px] uppercase tracking-wider text-gray-300 mb-10 flex items-center italic">
                   Audience Matrix
                </h3>
                ...
                <div className="flex justify-center gap-10 mt-6 border-t border-gray-50 pt-6">
                   <div className="flex items-center text-[10px] font-semibold text-sm text-gray-400"><div className="w-3 h-3 bg-blue-600 rounded-full mr-3 shadow-lg shadow-blue-500/30"></div> Male</div>
                   <div className="flex items-center text-[10px] font-semibold text-sm text-gray-400"><div className="w-3 h-3 bg-black rounded-full mr-3 shadow-lg shadow-black/30"></div> Female</div>
                </div>
             </div>
                ...
                <div className="space-y-6">
                   {groupLocations(data.locations).map((loc, i) => (
                     <div key={i} className="space-y-2.5">
                        <div className="flex justify-between text-[10px] font-semibold text-sm">
                   ...
                   {(!data.locations || data.locations.length === 0) && (
                     <div className="py-20 text-center">
                        ...
                        <p className="text-[10px] font-bold text-gray-300 uppercase tracking-normal italic">Regional data decryption pending...</p>
                     </div>
                   )}
                </div>
             </div>
          ...
          <div className="bg-white border border-gray-100 rounded-xl overflow-hidden shadow-2xl mt-12 mb-20">
             <div className="bg-black px-10 py-8 flex justify-between items-center">
               <h3 className="font-bold text-white flex items-center tracking-tight text-xl uppercase italic">Campaign Registry</h3>
               <div className="px-5 py-2 bg-white/10 rounded-xl text-[10px] font-bold text-blue-600 uppercase tracking-normal border border-white/5">
               ...
               <table className="w-full text-left border-collapse">
                 <thead>
                   <tr className="bg-gray-50/50 border-b border-gray-100">
                     <th className="px-10 py-6 font-bold text-[10px] text-gray-400 uppercase tracking-normal">Asset Descriptor</th>
                     <th className="px-10 py-6 font-bold text-[10px] text-gray-400 uppercase tracking-normal text-right">Investment</th>
                     <th className="px-10 py-6 font-bold text-[10px] text-gray-400 uppercase tracking-normal text-right">Reach</th>
                     <th className="px-10 py-6 font-bold text-[10px] text-gray-400 uppercase tracking-normal text-right">Engagement</th>
                     <th className="px-10 py-6 font-bold text-[10px] text-gray-400 uppercase tracking-normal text-center">Protocol Status</th>
                   </tr>
                 </thead>
                   ...
                   {data.campaigns?.map((camp: any, i: number) => (
                      <tr key={i} className="hover:bg-red-50/30 transition-all group">
                         <td className="px-10 py-6">
                            <p className="font-bold text-gray-900 group-hover:text-blue-600 transition-colors text-xs uppercase tracking-tight italic">{camp.campaign_name}</p>
                            <p className="text-[8px] text-gray-300 font-semibold text-sm mt-1">NODE_ID: {camp.id.slice(0,12)}...</p>
                         </td>
                         <td className="px-10 py-6 font-bold text-gray-900 text-right text-xs italic">${parseFloat(camp.spend || '0').toLocaleString(undefined, {minimumFractionDigits:2})}</td>
                         <td className="px-10 py-6 text-gray-400 font-bold text-right text-[10px] uppercase tracking-normal">{parseInt(camp.impressions || 0).toLocaleString()}</td>
                         <td className="px-10 py-6 text-gray-400 font-bold text-right text-[10px] uppercase tracking-normal">{parseInt(camp.clicks || 0).toLocaleString()}</td>
                         <td className="px-10 py-6 text-center">
                           <span className={`inline-flex items-center px-4 py-2 rounded-xl text-[9px] font-semibold text-sm border transition-all ${
                             camp.status === 'ACTIVE' ? 'bg-black text-white border-black' : 'bg-gray-50 text-gray-400 border-gray-100'
                           }`}>
                             {camp.status === 'ACTIVE' && <div className="w-1.5 h-1.5 rounded-full bg-blue-600 mr-2.5 animate-pulse"></div>}
                             {camp.status}
                           </span>
                         </td>
                      </tr>
...
function MetricCard({ title, value, icon, change, highlight, subtitle, link }: any) {
  ...
  const CardContent = (
    <div className={`p-8 rounded-xl border transition-all duration-500 hover:shadow-2xl group relative overflow-hidden ${highlight ? 'bg-black border-black text-white' : 'bg-white border-gray-100 shadow-sm'}`}>
       {highlight && <div className="absolute top-0 right-0 w-32 h-32 bg-blue-600 rounded-full blur-[80px] opacity-20 -mr-16 -mt-16 group-hover:scale-125 transition-transform"></div>}
       <div className="flex justify-between items-start mb-6 relative z-10">
         <p className={`font-bold text-[9px] uppercase tracking-[0.2em] ${highlight ? 'text-gray-500' : 'text-gray-400'}`}>{title}</p>
       ...
       <h4 className={`text-3xl font-bold tracking-tight mb-3 italic relative z-10 ${highlight ? 'text-white' : 'text-gray-900'}`}>{value}</h4>
       <div className="flex items-center justify-between relative z-10">
          <p className="text-[9px] font-bold text-gray-400 uppercase tracking-normal italic">{subtitle || (change !== undefined && `Vs Prev cycle: ${isPositive ? '↑' : '↓'} ${Math.abs(change).toFixed(1)}%`)}</p>
          {link && <ArrowRight className="w-4 h-4 text-blue-600 opacity-0 group-hover:opacity-100 translate-x-4 group-hover:translate-x-0 transition-all" />}
       </div>
    </div>
  )
