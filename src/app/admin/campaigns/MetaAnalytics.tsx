...
export default function MetaAnalytics({ clients }: { clients: ProjectClient[] }) {
  ...
  return (
    <div className="bg-white border text-sm border-gray-100 shadow-2xl rounded-xl p-8 mb-8 mt-2 relative overflow-hidden">
      {/* Background Accent */}
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-10 relative z-10">
        <div className="flex items-center gap-4">
           ...
           <div>
             <h2 className="text-2xl font-bold text-gray-900 tracking-tight font-bold">Campaign Intelligence</h2>
             <p className="text-gray-400 text-[10px] font-semibold text-sm mt-1">Global Meta Performance Engine</p>
           </div>
        </div>
      ...
      {isLoading ? (
        <div className="py-32 flex flex-col items-center justify-center text-center">
          ...
          <p className="mt-8 text-[10px] font-bold uppercase tracking-wider text-gray-400 animate-pulse">Initializing Neural Sync...</p>
        </div>
      ) : needsOauth ? (
        <div className="py-24 flex flex-col items-center justify-center text-center bg-red-50/30 rounded-xl border border-red-50 mx-auto max-w-2xl px-12 relative overflow-hidden">
          <div className="absolute -left-10 -top-10 w-32 h-32 bg-red-100 rounded-full blur-2xl opacity-50"></div>
          <div className="w-20 h-20 bg-blue-600 rounded-3xl shadow-xl shadow-blue-100 flex items-center justify-center mb-8 rotate-3 hover:rotate-0 transition-transform duration-500">
            <Target className="w-10 h-10 text-white" />
          </div>
          <h3 className="text-3xl font-bold text-gray-900 mb-4 tracking-tight font-bold">Meta Auth Required</h3>
          ...
          <button 
            ...
            className="bg-black hover:bg-blue-600 text-white px-12 py-5 rounded-2xl text-[10px] font-semibold text-sm flex items-center shadow-2xl shadow-gray-200 hover:shadow-blue-200 transition-all active:scale-95"
          >
            Authenticate Portal
          </button>
        </div>
        ...
        <div className="py-20 flex flex-col items-center justify-center text-center bg-red-50/50 rounded-3xl border border-red-100 max-w-2xl mx-auto">
          ...
          <button onClick={() => window.location.reload()} className="mt-6 text-[10px] font-semibold text-sm text-red-400 hover:text-red-600">Retry Connection</button>
        </div>
      ) : data ? (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
          {/* Main Metrics KPI Grid */}
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
            <MetricCard title="Capital Spent" value={`$${spend.toFixed(2)}`} icon={<TrendingUp className="text-blue-600"/>} highlight />
            <MetricCard title="Total Impressions" value={impressions.toLocaleString()} icon={<Users className="text-gray-900"/>} />
            <MetricCard title="Engagement" value={clicks.toLocaleString()} icon={<MousePointerClick className="text-blue-600"/>} />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
             ...
             <div className="lg:col-span-2 bg-white border border-gray-100 p-8 rounded-xl shadow-sm relative group overflow-hidden">
               <div className="absolute top-0 right-0 w-32 h-32 bg-red-50 rounded-full blur-3xl opacity-20 -mr-16 -mt-16"></div>
               <h3 className="text-[10px] font-bold text-gray-400 uppercase tracking-[0.2em] mb-10 flex items-center relative z-10">
                 <Activity className="w-5 h-5 mr-3 text-blue-600" /> Performance Trajectory
               </h3>
             ...
             <div className="bg-black p-8 rounded-xl shadow-xl text-white relative overflow-hidden group">
               <div className="absolute -right-10 -bottom-10 w-40 h-40 bg-blue-600 rounded-full blur-3xl opacity-20 group-hover:opacity-30 transition-opacity"></div>
               <h3 className="text-[10px] font-semibold text-sm mb-10 text-gray-400">Audience Architecture</h3>
               <div className="h-72 w-full relative z-10">
                  <ResponsiveContainer width="100%" height="100%">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="bg-white border border-gray-100 rounded-xl overflow-hidden shadow-sm">
               <div className="bg-gray-50/50 px-8 py-5 border-b border-gray-100">
                 <h3 className="text-[10px] font-semibold text-sm text-gray-900 flex items-center">Elite Creatives</h3>
               </div>
               <div className="divide-y divide-gray-50 p-4">
                 {data.ads?.slice(0, 6).map((ad: any, i: number) => (
                    <div key={i} className="px-4 py-4 flex justify-between items-center group hover:bg-red-50 rounded-2xl transition-all cursor-default">
                       <p className="font-bold text-gray-900 truncate w-2/3 group-hover:text-blue-600 transition-colors">{ad.ad_name}</p>
                       <p className="font-bold text-gray-900">${parseFloat(ad.spend || '0').toFixed(2)}</p>
                    </div>
                 ))}
                 {!data.ads?.length && <div className="p-12 text-center text-[10px] font-semibold text-sm text-gray-300">No creative data available</div>}
               </div>
            </div>
            ...
            <div className="bg-white border border-gray-100 rounded-xl overflow-hidden shadow-sm">
               <div className="bg-gray-50/50 px-8 py-5 border-b border-gray-100">
                 <h3 className="text-[10px] font-semibold text-sm text-gray-900 flex items-center">Global Delivery Nodes</h3>
               </div>
               <div className="divide-y divide-gray-50 p-4 h-[340px] overflow-y-auto custom-scrollbar">
                 {data.campaigns?.map((camp: any, i: number) => (
                    <div key={i} className="px-4 py-4 flex justify-between items-center group hover:bg-red-50 rounded-2xl transition-all">
                       <div className="w-2/3">
                         <p className="font-bold text-gray-900 truncate group-hover:text-blue-600 transition-colors capitalize">{camp.campaign_name.toLowerCase()}</p>
                         <p className="text-[10px] font-semibold text-sm text-gray-400 mt-1">{parseInt(camp.impressions || 0).toLocaleString()} reach nodes</p>
                       </div>
                       <span className={`px-3 py-1.5 rounded-xl text-[10px] font-semibold text-sm border ${
                 ...
                 {!data.campaigns?.length && <div className="p-12 text-center text-[10px] font-semibold text-sm text-gray-300">No active trajectories</div>}
               </div>
            </div>
          </div>
        </div>
  ...
  return (
    <div className={`p-6 rounded-3xl border transition-all hover:shadow-lg ${highlight ? 'bg-red-50/30 border-red-100' : 'bg-white border-gray-50 shadow-sm'}`}>
       <div className="flex justify-between items-start mb-4">
         <p className={`text-[10px] font-semibold text-sm ${highlight ? 'text-blue-600' : 'text-gray-400'}`}>{title}</p>
         {icon && <div className="p-2.5 bg-white rounded-2xl shadow-sm ring-1 ring-gray-100">{icon}</div>}
       </div>
       <h4 className={`text-3xl font-bold tracking-tight ${highlight ? 'text-gray-900' : 'text-gray-900'}`}>{value}</h4>
    </div>
  )
}
